    ...
    def __run__(self):
        if self.method == 'LOFO':
            ...
            for name in self.models_:
                ...
                self.results_table.loc[len(self.results_table.index)] = [f'{name}', 'GKF_CV_LOFO', np.average(cv_RMSE_), np.average(cv_R2)]
                self.results_table.loc[len(self.results_table.index)] = [f'{name}', 'GKF_TEST_LOFO', test_RMSE, test_R2]
